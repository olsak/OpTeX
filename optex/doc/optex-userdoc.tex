...
is reset in each section by default. You can declare \^`\swapeqno` if you
want to put equation marks to the opposite side.
...
\^`\picheight` register (or \^`\pich` in short)
which denotes the height of the picture. If both
...
position. The right edge is used if the optional parameter `[r]` is given, see
section~\ref[graphics] for more details.
...
\def\t#1{\ea\tA\ea{\string#1}\edef\tmp{\csstring#1}\iindex{;\tmp}\dest[cs:\tmp]\sxdef{cs:\tmp}{}}
\def\tA#1{\ea\ulink\ea[\tpgurl]{\tt#1}}
...
\tpg[ccd]\t\ccd\
converts color data from various formats to the \OpTeX/ format.
...
\tpg[inspicignore]\t\inspicignore\
the \^`\inspic` commands stop loading images, they are replaced by gray frames.

\tpg[inspicto]\t\inspicto\
behaves like \^`\inspic`, but saves PDF size when image is printed repeatedly.

\tpg[inspicwarn]\t\inspicwarn\
sets \^`\inspic` to print warning only if the image file isn't found.
...
\tpg[overlap]\t\overlap\
additional margins (for crop marks, for example).
...
\tpg[treedata]\t\treedata, \t\tracingtreedata, \tpg[treenodes]\t\treenodes\
\tpg[showtree]\t\showtree, \tpg[jsonread]\t\jsonread,
...
\tpg[verbhyphens]\t\verbhyphens\
setting breakable points for inline verbatim.
...
\^`\adef` `<char>{<body>}` defines <char> active character with meaning <body>. See also \^`\mathadef`.\nl
...
\^`\_newifi` `\_iffoo` declares boolean `\_iffoo`,
   \^`\_newifp\.iffoo` declares boolean `\.iffoo`\nl

...
subsripts is processed in $S$ style and the second and more levels of
exponents or indexes is processed in $SS$ style.
...
characters from more fonts (Unicode fonts or old TFM fonts together) in one math formula.
...
used for `\script` font and family 5 and more can be used by user for
loading more fonts. The macro for loading math fonts  looks like:
...
The example in section~\ref[newfam] show how to do it.

\secc Two variants of math fonts: normal and bold

The math formulas in the document need only one Unicode font (or only one collection
of math fonts as mentioned in section~\ref[fams]). But this
is not realy true if titles are in bold font. If math formula is present in
a such bold title then all characters of this formula must be bolder.
For example \"normal" variables must be in bold italic in titles, symbols
like `+` `=` must be bold and \"normal bold" letters (e.g., indicating vectors in math formula) must be
extra bold in titles.
...
`\loadboldmath` is used in second case. But if the bold variant of font is
not available (this is unfortunatelly more typical), then \OpTeX/ uses faked
...
Finally, we must to declare new \TeX/ sequences for acessing the characters

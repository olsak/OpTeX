...
italic, `\bf` selects upright bold font, `\bi` selects bold italic and `\tt` selects
monospaced font, `\em` select italic (if upright is current) or upright
...
* `\ref[<label>]` should create an interlnal link to the place given by
  ...
  if `\label[<label>]` precedes this control sequence or `[<label>]` is used
  as the parameter of this control sequence. The visual aspect of the link
